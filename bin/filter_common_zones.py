...
def zonesOverlap(zonesOrg, zonesCompList, indexZonesCompList, genomeComp):
	"""
		...
		:param indexZonesCompList: The Amont and Aval index of the zones of one accession
		:type indexZonesCompList: dict
	...
	# 1- Identification of potential identical regions based on common amont and aval chromosomes
	if amontChr in indexZonesCompList['amont'] and avalChr in indexZonesCompList['aval']:
		IdentAmontAndAval = indexZonesCompList['amont'][amontChr] & indexZonesCompList['aval'][avalChr]
		
		# 2- Recording the mate zone to test (amont and aval) overlapped positions
		if IdentAmontAndAval:
			
			# 3- Recording the candidat mate zone (amont and aval) overlapped positions
			for candidat in IdentAmontAndAval:
				CandamontChr = zonesCompList[candidat][0]
				CandamontStart = int(zonesCompList[candidat][1])
				CandamontEnd = int(zonesCompList[candidat][2])

				CandavalChr = zonesCompList[candidat][5]
				CandavalStart = int(zonesCompList[candidat][6])
				CandavalEnd = int(zonesCompList[candidat][7])
				
				if amontChr != amontChr:
					sys.exit('bug!')
				if avalChr != avalChr:
					sys.exit('bug!')
						
				if amontStart <= CandamontStart and CandamontStart <= amontEnd:
					if avalStart <= CandavalStart and CandavalStart <= avalEnd:
						overlap = True
						if zonesCompList[candidat][13] == "PASSED":
							zonePassed = True
						elif zonesCompList[candidat][13] == "NOT_PASSED":
							zoneNotPassed = True
					elif avalStart <= CandavalEnd and CandavalEnd <= avalEnd:
						overlap = True
						if zonesCompList[candidat][13] == "PASSED":
							zonePassed = True
						elif zonesCompList[candidat][13] == "NOT_PASSED":
							zoneNotPassed = True
					elif CandavalStart <= avalStart and avalStart <= CandavalEnd:
						overlap = True
						if zonesCompList[candidat][13] == "PASSED":
							zonePassed = True
						elif zonesCompList[candidat][13] == "NOT_PASSED":
							zoneNotPassed = True
					elif CandavalStart <= avalEnd and avalEnd <= CandavalEnd:
						overlap = True
						if zonesCompList[candidat][13] == "PASSED":
							zonePassed = True
						elif zonesCompList[candidat][13] == "NOT_PASSED":
							zoneNotPassed = True
				elif amontStart <= CandamontEnd and CandamontEnd <= amontEnd:
					if avalStart <= CandavalStart and CandavalStart <= avalEnd:
						overlap = True
						if zonesCompList[candidat][13] == "PASSED":
							zonePassed = True
						...
						if zonesCompList[candidat][13] == "PASSED":
							...
							zoneNotPassed = True
				elif CandamontStart <= amontStart and amontStart <= CandamontEnd:
					if avalStart <= CandavalStart and CandavalStart <= avalEnd:
						overlap = True
						if zonesCompList[candidat][13] == "PASSED":
							zonePassed = True
						...
						if zonesCompList[candidat][13] == "PASSED":
							...
							zoneNotPassed = True
				elif CandamontStart <= amontEnd and amontEnd <= CandamontEnd:
					if avalStart <= CandavalStart and CandavalStart <= avalEnd:
						overlap = True
						if zonesCompList[candidat][13] == "PASSED":
							zonePassed = True
...
def findZoneInBam(locaPrograms, bamAmont, bamAval, zones, nbReadsMax, margin, TMP):
	"""
		...
		:type margin: int
		:param TMP: A uniq prefix for output (prevent to generate a large amount of temporary files which will be removed several time).
		:type TMP: str
	...
	tmp_amont_subsam = TMP+'_tmp_amont_subsam'
	tmp_aval_subsam = TMP+'_tmp_aval_subsam'

	# 1- Getting reads mapping at zone1 and zone2
	zone = []  # to return
	...
	# 2- Getting read pairs mapping at both zone1 and zone2
	ReadsCommons = intersectReads(locaPrograms, tmp_amont_subsam, tmp_aval_subsam)
	nbReadsCommons = len(ReadsCommons)

	if nbReadsCommons > nbReadsMax:

		FileCommons = TMP+'_ReadsCommons.tab'
		outfile = open(FileCommons, 'w')
		for reads in ReadsCommons:
			outfile.write(reads+'\n')
		outfile.close()

		tmp_Common_amont_subsam = TMP+'_tmp_Common_amont_subsam'
		tmp_Common_aval_subsam = TMP+'_tmp_Common_aval_subsam'

		utils.extractSamFromReads(locaPrograms, tmp_amont_subsam, 'bam', FileCommons, tmp_Common_amont_subsam)
		utils.extractSamFromReads(locaPrograms, tmp_aval_subsam, 'bam', FileCommons, tmp_Common_aval_subsam)

		# 3- Calculating borders
		zoneAmont = calculBorder(locaPrograms, tmp_Common_amont_subsam, TMP)
		zoneAval = calculBorder(locaPrograms, tmp_Common_aval_subsam, TMP)

		# 4- returning the zone
...
def intersectReads (locaPrograms, samAmont, samAval):
	"""
		...
		:type samAval: str
		:return: The list of common reads
		:rtype: int
	"""
	...
	if readsAmont:
		...
		if readsAval:
			...
			setListAmont = set(listReadsAmont)
			intersect = setListAval & setListAmont

	return intersect

def calculBorder(locaPrograms, sam, TMP):
	"""
...
def lookForCommonZones(locaPrograms, genomesList, discType):
	...
	#---------------------------added by Guillaume------------------------------------------------
	listZonesIndex = {}  # A dict containing a chromosome amont/aval index of each zones by genome
	#---------------------------------------------------------------------------------------------
	...
	# Indexing amont and aval chromosomes (added by Guillaume)
	#----------------------------------------------------------------------
	for genome in listZones:
		i = 0
		listZonesIndex[genome] = {}
		listZonesIndex[genome]['amont'] = {}
		listZonesIndex[genome]['aval'] = {}
		for mateZones in listZones[genome]:
			if mateZones:
				if not mateZones[0] in listZonesIndex[genome]['amont']:
					listZonesIndex[genome]['amont'][mateZones[0]] = set()
				listZonesIndex[genome]['amont'][mateZones[0]].add(i)
				if not mateZones[5] in listZonesIndex[genome]['aval']:
					listZonesIndex[genome]['aval'][mateZones[5]] = set()
				listZonesIndex[genome]['aval'][mateZones[5]].add(i)
			i += 1
	#----------------------------------------------------------------------
	
	# Search for similar regions in other species
	for genome in genomesList:
		nbZones[genome] = {}
		i = 0
		for mateZones in listZones[genome]:
			if mateZones:
				...
				for genomeComp in genomesList:
					if genomeComp != genome:
						zonesFound = zonesOverlap(mateZones, listZones[genomeComp], listZonesIndex[genomeComp], genomeComp)
						if zonesFound[0]:
							if zonesFound[1]:
								flag = "3"
							else:
...
def __main__():
	...
	"Minor editing performed by Guillaume Martin : guillaume.martin@cirad.fr")
