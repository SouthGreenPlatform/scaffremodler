...
import optparse, os, shutil, subprocess, sys, tempfile, fileinput, ConfigParser, operator, time, math, glob, datetime, pysam
...
def BamGrepReadsBam (LISTE, BAM, OUT):
	file = open(LISTE)
	dic = set()
	for line in file:
		data = line.split()
	...
	samread = pysam.AlignmentFile(OUT, "wb", template=samfile)
	for read in samfile.fetch():
		if read.query_name in dic:
			samread.write(read)
	samfile.close()
	samread.close()

def BamGrepReadsSam (LISTE, BAM, OUT):
	file = open(LISTE)
	dic = set()
	for line in file:
		data = line.split()
	...
	file.close()
	samfile = pysam.AlignmentFile(BAM, "rb")
	samread = pysam.AlignmentFile(OUT, "wh", template=samfile)
	for read in samfile.fetch():
		if read.query_name in dic:
			samread.write(read)
	samfile.close()
...
def calculCovFromMateZones(LOCA_PROGRAMS, bamAmont, bamAval, TYPE, CHR_Amont, START_Amont, END_amont, CHR_Aval, START_Aval, END_Aval):
	...
	#Indexing bam files
	index_bam_file(LOCA_PROGRAMS, 'bam_amont_tmp')
	index_bam_file(LOCA_PROGRAMS, 'bam_aval_tmp')

	#create two bam files for each zones with only the common reads
	BamGrepReadsBam('commonReadsInMateZones_tmp', 'bam_amont_tmp', 'sam_amont_selected_tmp')

	BamGrepReadsBam('commonReadsInMateZones_tmp', 'bam_aval_tmp', 'sam_aval_selected_tmp')

	# calcul of the coverage of this two sam file
	calcul_cov(LOCA_PROGRAMS, 'sam_amont_selected_tmp', 'bam', 'cov_amont_tmp')
	...
	calcul_cov(LOCA_PROGRAMS, 'sam_aval_selected_tmp', 'bam', 'cov_aval_tmp')
		...
		if data:
			...
			while debut <= int(data[1]):
				...
				if os.path.getsize(tempo2.name) != 0:
					BamGrepReadsBam(tempo2.name, BAM, OUT+'_'+data[0]+'_'+str(debut)+'-'+str(fin)+'.bam')
...
def create_sub_sam(outLog, LOCA_PROGRAMS, BAM, CHR, START, END, OUT):
	...
	if os.path.getsize(tempo2.name) != 0:
		...
		BamGrepReadsSam(tempo2.name, BAM, OUT)
	else:
		logOutput = open(outLog, 'a')
...
def recalc_border(LOCA_PROGRAMS, LISTE, SAM, ZCOV, MAXCOV, MINCOV):
	...
	#Indexing bam file
	index_bam_file(LOCA_PROGRAMS, SAM+'_bam')

	#Create a sub-sam based on read list
	BamGrepReadsSam(LISTE, SAM+'_bam', SAM+'_sub.sam')
