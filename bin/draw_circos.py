...
def create_conf(DIC, CONF, IDEOGRAM, TICKS, CONFIG, OUT, CHR, ORDER, UNIT, COV, SCAFF, DISCORD, FRF, FF, RR, INS, DELET, CHR_RR, CHR_FR, CHR_RF, CHR_FF, READ_FR, READ_RF, READ_FF, READ_RR, READ_INS, READ_DELET, READ_CHR_RR, READ_CHR_RF, READ_CHR_FR, READ_CHR_FF, TEXT, KAR, LABEL):
	...
	outfile = open(CONF,'w')
	outfile.write('<<include '+configuration.get('General','out_housekeeping')+'>>\n')
	outfile.write('<<include etc/colors_fonts_patterns.conf>>\n')
	outfile.write('<colors>\n')
	outfile.write('<<include etc/colors.conf>>\n')
	outfile.write('zblue = 64,137,255\n')
	outfile.write('rblue = 38,82,153\n')
	outfile.write('zgreen = 128,255,164\n')
	outfile.write('rgreen = 38,153,71\n')
	outfile.write('zpurple = 236,64,255\n')
	outfile.write('rpurple = 118,32,128\n')
	outfile.write('zorange = 255,187,110\n')
	outfile.write('rorange = 221,143,55\n')
	outfile.write('zred = 255,122,137\n')
	outfile.write('rred = 205,51,69\n')
	outfile.write('rgrey = 200,200,200\n')
	outfile.write('</colors>\n')
	# outfile.write('<fonts>\n')
		...
		if TEXT == 'y' or (configuration.get('General','prop') == 'yes' and DISCORD == 'y') or (configuration.get('General','cov') == 'yes' and COV == 'y') or (configuration.get('General','scaff_tile') == 'yes' and SCAFF == 'y'):
			...
			if DISCORD == 'y':
				if configuration.get('General','prop') == 'yes':
					...
					outfile.write('<axes>\n')
					outfile.write('<axis>\n')
					outfile.write('color     = lgrey\n')
					outfile.write('thickness = 1\n')
					outfile.write('spacing   = 0.25\n')
					outfile.write('</axis>\n')
					outfile.write('</axes>\n')
					outfile.write('<rules>\n')
					outfile.write('<rule>\n')
					outfile.write('importance = 100\n')
					outfile.write('condition    = var(value) < 0\n')
					outfile.write('color        = black\n')
					outfile.write('fill_color   = black\n')
					outfile.write('glyph        = circle\n')
					outfile.write('glyph_size   = 5\n')
					...
					outfile.write('condition    = var(value) < 0.25\n')
					...
					outfile.write('condition    = var(value) < 0.50\n')
					...
					outfile.write('<rule>\n')
					outfile.write('importance = 80\n')
					outfile.write('condition    = var(value) < 0.75\n')
			...
			if COV == 'y':
				if configuration.get('General','cov') == 'yes':
					...
					outfile.write('color     = black\n')
					outfile.write('thickness = 1\n')
					# outfile.write('spacing   = %s\n' % str(configuration.getfloat('Coverage','median_cov')*6))
					outfile.write('position = 0.25r\n')
					outfile.write('</axis>\n')
					outfile.write('</axes>\n')
					outfile.write('<rules>\n')
					outfile.write('<rule>\n')
					outfile.write('importance = 100\n')
					outfile.write('condition    = var(value) > %s\n' % str(configuration.getfloat('Coverage','mean_cov')+configuration.getfloat('Coverage','median_cov')))
					outfile.write('color        = red\n')
					outfile.write('fill_color   = red\n')
					outfile.write('glyph        = circle\n')
					outfile.write('glyph_size   = 5\n')
...
def cree_link_final(DIC, LINK, OUT):
	...
	for line in file:
		if line.strip():
			cols = line.split()
			if cols[0] in DIC and cols[3] in DIC:
				dedans = 0
				for j in DIC[cols[0]]:
					if int(cols[1]) >= int(j[0]) and int(cols[2]) <= int(j[1]):
						dedans = 1
						break
				for j in DIC[cols[3]]:
					if int(cols[4]) >= int(j[0]) and int(cols[5]) <= int(j[1]) and dedans == 1:
						dedans = 2
						break
				if dedans == 2:
					outfile.write(line)
...
def __main__():
	...
	parser.add_option( '', '--map', dest='map', default='n', help='Create html map of the circos.')
	...
	if options.map == 'n':
		run_circos = '%s --conf %s -noparanoid' % (loca_programs.get('Programs','circos'), conf, )
	else:
		run_circos = '%s --conf %s -noparanoid -param image/image_map_use=yes -param image/image_map_file=%s' % (loca_programs.get('Programs','circos'), conf, os.path.splitext(options.out)[0])
