...
def define_regions(DRAW, CHR):
	"""
		Calcul the zone to draw and their order.
		
		:param DRAW: A list of chromosome position separated with = (ex:chr01=1000=20000)
		:type DRAW: str
		:param CHR: A file contraining the list of all the chromosomes and their length
		:type CHR: str
		:return: A list like : [chr_name, chr_order, unit, dic]
		:rtype: list
	"""
		...
		if data:
			if data[0] in dic:
				...
				for n in sorted_liste:
					if char > 25: # 25 due to there is only 24 letter in the alphabet
...
def __main__():
	...
	parser.add_option( '', '--nbaccess', dest='nbaccess', default='0', help='Draw zone presents in n accessions, [default: %default]')
