...
def IntsToPoints(ints):
	...
	for i in range(0, len(ints), 2):
		result.append(Point(int(ints[i]), int(ints[i+1])))
	return result
def Area(polygon):
...
def _StripDupPts(poly):
	...
	for i, p in enumerate(poly):
		if i > 0 and not _PointsEqual(poly[i-1], p) and not _PointsEqual(p, poly[0]):
			newPoly.append(p)
	return newPoly
	...
	def handle_starttag(self, tag, attrs):
		...
		if tag == 'area':
			for name, value in attrs:
				if name == "coords":
					poly = IntsToPoints(value.split(','))
					subj = [poly]
					if self.margin < 0 or self.margin > 1000000:
						c = OffsetPolygons(subj, 2.0)
						c = OffsetPolygons(c, 3.0)
					else:
						c = subj
				elif name == "href":
					h = value
			self.coords.append([c[0], h])
		elif tag == "map":
			for name, value in attrs:
				if name == "name":
					self.mapName = value
...
def getCoords(parser):
	return [parser.mapName, parser.coords]

def createHtmlTemplate(accessionName, configFile, chrList):

	html = '<!DOCTYPE html> \n\
	<html> \n\
		<head> \n\
			<meta charset="UTF-8"> \n\
			<title>Scaffremodler visualization</title> \n\
			<link rel="stylesheet" href="template/css/styles.css" media="all"> \n\
		</head> \n\
		<body> \n\
			<div class="overlay"></div>\n\
			<div class="headerBand"> \n\
				<span class="toogleHeader alignRight toggleHide"></span> \n\
				<div class="containerBlockUI"> \n\
					<div class="span_3_of_12_nm titleDashboard nomargin"> \n\
						<h1>Scaffremodler Visualization</h1> \n\
					</div> \n\
					<div class="span_3_of_12_nm header nomargin"> \n\
							<div class="accessiontitle"> \n\
								<h2>Accession name</h2> \n\
								<span>'+accessionName+'</span>\n\
							</div>\n\
					</div>\n\
					<div class="header nomargin">\n\
						<h2>Circos legend</h2>\n\
						<div class="legendContainer">\n\
							<div class="legendBlock lblue"><span></span>Reverse Orientation</div>\n\
							<div class="legendBlock lgreen"><span></span>Forward/Forward</div>\n\
							<div class="legendBlock lpurple"><span></span>Reverse/Reverse</div>\n\
							<div class="legendBlock lred"><span></span>Higher insert size</div>\n\
							<div class="legendBlock lorange"><span></span>Lower insert size</div>\n\
							<div class="legendBlock lgrey"><span></span>Concordant read</div>\n\
						</div>\n\
						<div class="legendContainer legendCircles">\n\
							<img src="template/img/legend2.png" alt="" />\n\
								<div class="labelLegendCircles"><span>&#125; Coverage</span><span>&#125; Discordant reads</span><span>&#125; Reference</span></div>\n\
						</div>\n\
					</div>\n\
				</div>\n\
			</div>\n\
			<div class="section_nm group_nm">\n\
				<div class="col_nm span_3_of_12_nm">\n\
					<div class="blockui">\n\
						<span class="alignRight toggleHideBtn toggleHide"></span>\n\
						<h2>Select your chromosomes</h2>\n\
						<div class="containerBlockUI">\n\
							<div class="menuChr">\n\
								<select name="selectChr1" id="selectChr1">\n\
								</select>\n\
								vs\n\
								<select name="selectChr2" id="selectChr2">\n\
								</select>\n\
							</div>\n\
					...
					<div id="menuImg" class="blockui">\n\
						<span class="alignRight toggleHideBtn toggleHide"></span>\n\
						<h2>Image zoom : </h2>\n\
						<div class="containerBlockUI">\n\
							<input id="zoom" name="zoom" type="range" min="0" max="100" value="0" step="5" data-highlight="true"/>\n\
						</div>\n\
					</div>\n\
					<div class="informationContainer blockui">\n\
						<span class="alignRight toggleHideBtn toggleHide"></span>\n\
						<h2><div class="tooltipContainer"><span class="tooltip">Information of the zone :</span><span>Click on a zone in the circos image.</span></div></h2>\n\
						<div class="containerBlockUI">\n\
							<ul>\n\
								<li class="colonne"><p>First chromosome : </p><p id="chr1" class="spanInfo"></p></li>\n\
								<li class="colonne"><p>First start position : </p><p id="start1" class="spanInfo"></p></li>\n\
								<li class="colonne"><p>First end position : </p><p id="end1" class="spanInfo"></p></li>\n\
								<li class="colonne"><p>Size of the first zone : </p><p id="size1" class="spanInfo"></p></li>\n\
								<li class="colonne"><p>Coverage of the first zone : </p><p id="cov1" class="spanInfo"></p></li>\n\
								<li class="colonne"><p>Second chromosome : </p><p id="chr2" class="spanInfo"></p></li>\n\
								<li class="colonne"><p>Second start position : </p><p id="start2" class="spanInfo"></p></li>\n\
								<li class="colonne"><p>Second end position : </p><p id="end2" class="spanInfo"></p></li>\n\
								<li class="colonne"><p>Size of the second zone : </p><p id="size2" class="spanInfo"></p></li>\n\
								<li class="colonne"><p>Coverage of the second zone : </p><p id="cov2" class="spanInfo"></p></li>\n\
								<li class="colonne"><p>Number of reads : </p><p id="reads" class="spanInfo"></p></li>\n\
							</ul>\n\
						</div>\n\
					</div>\n\
					<div class="blockui hidden">\n\
						<span class="alignRight toggleHideBtn toggleHide"></span>\n\
						<h2>Presents in the accessions :</h2>\n\
						<div class="containerBlockUI">\n\
							<div id="accessContainer">\n\
							</div>\n\
						</div>\n\
					</div>\n\
					<div id="circosCommande" class="blockui">\n\
						<div class="alignRight"><div id="cross"></div></div><span class="alignRight toggleHideBtn toggleHide"></span>\n\
						<h2><div class="tooltipContainer"><span class="tooltip">Circos Command :</span><span>Command to draw the specific zone selected.</span></div></h2>\n\
						<div class="containerBlockUI">\n\
							<div class="alignLeft aboveTextarea moreOptions">More command options</div>\n\
							<div class="optionsCircosCommands">\n\
								<h3>Kind of links to draw : <span class="linkinTitle selectZones">select all zones links</span><span class="linkinTitle unselectZones">unselect all zones links</span> | <span class="linkinTitle selectReads">select all reads links</span><span class="linkinTitle unselectReads">unselect all reads links</span></h3>\n\
								<div class="formGroup alignLeft zoneLinks">\n\
									<div class="alignLeft">Zones : </div>\n\
									<div class="alignLeft">\n\
										<div>\n\
											<input type="checkbox" name="zone_ff" id="zone_ff" value="y" checked><label for="zone_ff">Forward/Forward</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="zone_rr" id="zone_rr" value="y" checked><label for="zone_rr">Reverse/Reverse</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="zone_fr" id="zone_fr" value="y" checked><label for="zone_fr">Reverse Orientation</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="zone_ins" id="zone_ins" value="y" checked><label for="zone_ins">Higher insert size</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="zone_del" id="zone_del" value="y" checked><label for="zone_del">Lower insert size</label>\n\
										</div>\n\
									</div>\n\
									<div class="alignLeft">\n\
										<div>\n\
											<input type="checkbox" name="zone_chr_ff" id="zone_chr_ff" value="y" checked><label for="zone_chr_ff">chr_Forward/Forward (inter chromosomal)</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="zone_chr_fr" id="zone_chr_fr" value="y" checked><label for="zone_chr_fr">chr_Reverse/Reverse (inter chromosomal)</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="zone_chr_rf" id="zone_chr_rf" value="y" checked><label for="zone_chr_rf">chr_Forward/Forward (inter chromosomal)</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="zone_chr_rr" id="zone_chr_rr" value="y" checked><label for="zone_chr_rr">chr_Reverse/Reverse (inter chromosomal)</label>\n\
										</div>\n\
									</div>\n\
								</div>\n\
								<div class="formGroup alignLeft zoneReads">\n\
									<div class="alignLeft">Reads : </div>\n\
									<div class="alignLeft">\n\
										<div>\n\
											<input type="checkbox" name="reads_ff" id="reads_ff" value="y"><label for="reads_ff">Forward/Forward</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="reads_rr" id="reads_rr" value="y"><label for="reads_rr">Reverse/Reverse</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="reads_fr" id="reads_fr" value="y"><label for="reads_fr">Reverse Orientation</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="reads_ins" id="reads_ins" value="y"><label for="reads_ins">Higher insert size</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="reads_del" id="reads_del" value="y"><label for="reads_del">Lower insert size</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="reads_rf" id="reads_rf" value="y"><label for="reads_rf">Concordant read</label>\n\
										</div>\n\
									</div>\n\
									<div class="alignLeft">\n\
										<div>\n\
											<input type="checkbox" name="reads_chr_ff" id="reads_chr_ff" value="y" ><label for="reads_chr_ff">Forward/Forward (inter chromosomal)</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="reads_chr_fr" id="reads_chr_fr" value="y" ><label for="reads_chr_fr">Reverse/Reverse (inter chromosomal)</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="reads_chr_rf" id="reads_chr_rf" value="y" ><label for="reads_chr_rf">Forward/Forward (inter chromosomal)</label>\n\
										</div>\n\
										<div>\n\
											<input type="checkbox" name="reads_chr_rr" id="reads_chr_rr" value="y" ><label for="reads_chr_rr">Reverse/Reverse (inter chromosomal)</label>\n\
										</div>\n\
									</div>\n\
								</div>\n\
								<h3>Other options :</h3>\n\
								<div class="clearFloat">\n\
									<div class="formfield">\n\
										<label for="threads" class="formfieldLabel">Number of thread to use :</label><input type="text" value="1" size="2" name="threads" id="threads" class="formfieldInput"/> |\n\
									</div>\n\
									<div class="formfield">\n\
										<label for="cov" class="formfieldLabel">Draw coverage plot :</label><input type="checkbox" value="1" name="cov" id="cov" class="formfieldInput" checked/> |\n\
									</div>\n\
									<div class="formfield">\n\
										<label for="scaffolds" class="formfieldLabel">Draw scaffolds positions :</label><input type="checkbox" value="1" name="scaffolds" id="scaffolds" class="formfieldInput"/> |\n\
									</div>\n\
									<div class="formfield">\n\
										<label for="discordant" class="formfieldLabel">Draw discordant proportion plot :</label><input type="checkbox" value="1" name="discordant" id="discordant" class="formfieldInput" checked/>\n\
									</div>\n\
								</div>\n\
							</div>\n\
							<textarea></textarea>\n\
							<div class="tooltipContainer margin"><span class="tooltip">Margin (in bp) :</span><span>Corresponds to the distance to draw from either side of the zone.</span><input type="text" id="margin" name="margin" value="10000" class="inputShort" size="10"/></div>\n\
							<div class="errorContainer"></div>\n\
						</div>\n\
					</div>\n\
				</div>\n\
				<div class="col_nm span_9_of_12_nm">\n\
					<div class="circosContainer blockui">\n\
						<div class="mapContent"></div>\n\
						<img src="" alt="circos" usemap="" id="circosImg" />\n\
					</div>\n\
				</div>\n\
			</div>\n\
			<span id="accession" class="hidden">'+accessionName+'</span> \n\
			<span id="confCircos" class="hidden">'+configFile+'</span> \n\
			<script src="template/js/jquery-2.1.4.min.js"></script>\n\
			<script src="template/js/jquery.imagemapster.js"></script>\n\
			<script src="template/js/scaff.js"></script>\n\
			<script src="template/js/areas.json"></script>\n\
			<script src="template/js/plist.json"></script>\n\
			<footer>\n\
				<p>Scaffremodler - CIRAD</p>\n\
			</footer>\n\
		</body>\n\
	</html>'

	with open(accessionName+'/scaffremodler.html', 'w') as htmlFile:
		htmlFile.write(html)

def createJS(outputFile, rsltJobs):
	if os.path.isfile(outputFile):
		with open(outputFile, 'r') as areaFile:
			lines = areaFile.readlines()
		jsonContent = ""
		for line in lines[1:]:
			jsonContent += line.strip()
		# print jsonContent
		data = json.loads(jsonContent)
	else:
		data = {}
	for rslt in rsltJobs:
		data[rslt[1][0]] = []
		for area in rslt[1][1]:
			coords = []
			for point in area[0]:
				coords.extend([point[0], point[1]])
			data[rslt[1][0]].append([coords, area[1]])
...
def drawCircos(job):
	errorCode = 0
	try:
		errorCode = run_job(getframeinfo(currentframe()), job, "error on the job : "+job+"\n")
...
def extendPolygon(job):
	htmlFile = job[0]
	margin = job[1]
	codeError = 0
	coords = []
	try:
		parser = MyHTMLParser(margin)
		f = open(htmlFile+".html", 'r').read()
		parser.feed(f)
		# writeHtml(parser, htmlFile+".html")
		coords = getCoords(parser)
		...
		return [codeError, coords]


def readExistingCircos(projectName):
	if os.path.isfile(projectName+'/template/js/plist.json'):
		with open(projectName+'/template/js/plist.json', 'r') as infile:
...
def __main__():
	...
	parser.add_argument( '--config', required=True, help='The configuration file generated by conf4circos.py.')
	parser.add_argument( '--name', required=True, help='The name of the accession.')
	parser.add_argument( '--chr', required=True, help='List of chromosoms to draw. (ex : chr01-chr02-chr03)')
	parser.add_argument( '--thread', default=1, help='The number of threads to use. (default: %(default)s)')
	parser.add_argument( '--versus', dest='versus', action='store_true', help='Draw each chromosomes against the others.  (default: False)')
	parser.add_argument( '--erase', dest='erase', action='store_true', help='Erase an existing project with the same name  (default: False)')
	parser.add_argument('--cov', dest='cov', default='y', help='Draw coverage plot (y or n), [default: %default]')
	parser.add_argument('--scaffolds', dest='scaffolds', default='n', help='Draw scaffolds positions (y or n), [default: %default]')
	parser.add_argument('--discordant', dest='discordant', default='y', help='Draw discordant proportion plot (y or n), [default: %default]')
	parser.add_argument('--margin', dest='margin', default='', help='Margin to add to each side of a zone, [default: %default]')

	parser.add_argument('--fr', dest='fr', default='y', help='Draw discordant frf link (y or n), [default: %default]')
	parser.add_argument('--ff', dest='ff', default='y', help='Draw discordant ff link (y or n), [default: %default]')
	parser.add_argument('--rr', dest='rr', default='y', help='Draw discordant rr link (y or n), [default: %default]')
	parser.add_argument('--ins', dest='ins', default='y', help='Draw discordant ins link (y or n), [default: %default]')
	parser.add_argument('--delet', dest='delet', default='y', help='Draw discordant delet link (y or n), [default: %default]')
	parser.add_argument('--chr_rr', dest='chr_rr', default='y', help='Draw discordant chr_rr link (y or n), [default: %default]')
	parser.add_argument('--chr_rf', dest='chr_rf', default='y', help='Draw discordant chr_rf link (y or n), [default: %default]')
	parser.add_argument('--chr_fr', dest='chr_fr', default='y', help='Draw discordant chr_fr link (y or n), [default: %default]')
	parser.add_argument('--chr_ff', dest='chr_ff', default='y', help='Draw discordant chr_ff link (y or n), [default: %default]')

	parser.add_argument('--read_fr', dest='read_fr', default='n', help='Draw read fr link (y or n), [default: %default]')
	parser.add_argument('--read_rf', dest='read_rf', default='n', help='Draw read rf link (y or n), [default: %default]')
	parser.add_argument('--read_ff', dest='read_ff', default='n', help='Draw read ff link (y or n), [default: %default]')
	parser.add_argument('--read_rr', dest='read_rr', default='n', help='Draw read rr link (y or n), [default: %default]')
	parser.add_argument('--read_ins', dest='read_ins', default='n', help='Draw read ins link (y or n), [default: %default]')
	parser.add_argument('--read_del', dest='read_del', default='n', help='Draw read delet link (y or n), [default: %default]')
	parser.add_argument('--read_chr_rr', dest='read_chr_rr', default='n', help='Draw read chr_rr link (y or n), [default: %default]')
	parser.add_argument('--read_chr_rf', dest='read_chr_rf', default='n', help='Draw read chr_rf link (y or n), [default: %default]')
	parser.add_argument('--read_chr_fr', dest='read_chr_fr', default='n', help='Draw read chr_fr link (y or n), [default: %default]')
	parser.add_argument('--read_chr_ff', dest='read_chr_ff', default='n', help='Draw read chr_ff link (y or n), [default: %default]')
	...
	chrNum = len(chrList)
	threadsNb = int(options.thread)

	#check chromosomes names
	if not os.path.isfile(options.config):
		raise IOError("The conf4circos config file (param --config) is not found.")

	config = ConfigParser.RawConfigParser()
	config.read(options.config)

	#Check the chr to draw
	if not options.margin:
		margin = -1
	else:
		margin = int(options.margin)
	for chrom in sorted(options.chr.split('-')):
		chrom = chrom.split(':')
		if len(chrom) != 1 and len(chrom) != 3:
			err = 'Wrong format of the sequence to draw : '+':'.join(chrom)
			raise ValueError(err)
		elif len(chrom) == 3:
			chrom = [chrom[0], str(int(chrom[1])-margin), str(int(chrom[2])+margin)]
		else:
			chrom = [chrom[0]]
		chrList.append(chrom)
	with open(config.get('General', 'chr'), 'r') as chrFile:
		chrInFile = []
		for line in chrFile:
			if line.strip():
				chrInFile.append(line.split()[0])
		for chrom in chrList:
			if not chrom[0] in chrInFile:
				raise ValueError("The chromosom '"+str(chrom)+"' is not found in the file : "+config.get('General', 'chr')+".")

	if options.erase:
		projList = {}
	else:
		projList = readExistingCircos(options.name)

	#construct the linkfs to draw from options
	linksToDraw = ""
	linksToDraw += ('--ff y ' if options.ff == 'y' else '--ff n ')
	linksToDraw += ('--fr y ' if options.fr == 'y' else '--fr n ')
	linksToDraw += ('--rr y ' if options.rr == 'y' else '--rr n ')
	linksToDraw += ('--ins y ' if options.ins == 'y' else '--ins n ')
	linksToDraw += ('--delet y ' if options.delet == 'y' else '--delet n ')
	linksToDraw += ('--chr_ff y ' if options.chr_ff == 'y' else '--chr_ff n ')
	linksToDraw += ('--chr_fr y ' if options.chr_fr == 'y' else '--chr_fr n ')
	linksToDraw += ('--chr_rf y ' if options.chr_rf == 'y' else '--chr_rf n ')
	linksToDraw += ('--chr_rr y ' if options.chr_rr == 'y' else '--chr_rr n ')
	linksToDraw += ('--read_ff y ' if options.read_ff == 'y' else '--read_ff n ')
	linksToDraw += ('--read_fr y ' if options.read_fr == 'y' else '--read_fr n ')
	linksToDraw += ('--read_rf y ' if options.read_rf == 'y' else '--read_rf n ')
	linksToDraw += ('--read_rr y ' if options.read_rr == 'y' else '--read_rr n ')
	linksToDraw += ('--read_ins y ' if options.read_ins == 'y' else '--read_ins n ')
	linksToDraw += ('--read_del y ' if options.read_del == 'y' else '--read_del n ')
	linksToDraw += ('--read_chr_ff y ' if options.read_chr_ff == 'y' else '--read_chr_ff n ')
	linksToDraw += ('--read_chr_fr y ' if options.read_chr_fr == 'y' else '--read_chr_fr n ')
	linksToDraw += ('--read_chr_rf y ' if options.read_chr_rf == 'y' else '--read_chr_rf n ')
	linksToDraw += ('--read_chr_rr y ' if options.read_chr_rr == 'y' else '--read_chr_rr n ')

	otherOptions = ""
	otherOptions += ('--cov y ' if options.cov == 'y' else '--cov n ')
	otherOptions += ('--scaff y ' if options.scaffolds == 'y' else '--scaff n ')
	otherOptions += ('--discord y ' if options.discordant == 'y' else '--discord n ')

	if len(chrList) == 1:
		chr1 = ':'.join(chrom)
		listJobs.append("/usr/local/bioinfo/python/2.7.9/bin/python /home/derouault/Apps/scaffremodler-1.0/bin/draw_circos.py --config %s --draw %s --cov y --scaff n --text n --out %s_%s.png --map y %s %s" % (options.config, chr1, options.name, chr1.replace(':', '_'), linksToDraw, otherOptions))
		filesNames.append([options.name+"_"+chr1.replace(':', '_'), margin])
		if chr1.replace(':', '_') in projList:
			if not chr1.replace(':', '_') in projList[chr1.replace(':', '_')]:
				projList[chr1.replace(':', '_')] += [chr1.replace(':', '_')]
		else:
			projList[chr1.replace(':', '_')] = [chr1.replace(':', '_')]
	else:
		if options.versus:
			for i in range(0, len(chrList)):
				for j in range(i+1, len(chrList)):
					chr1 = ':'.join(chrList[i])
					chr2 = ':'.join(chrList[j])
					listJobs.append("/usr/local/bioinfo/python/2.7.9/bin/python /home/derouault/Apps/scaffremodler-1.0/bin/draw_circos.py --config %s --draw %s-%s --cov y --scaff n --text n --out %s_%s_%s.png --map y %s %s" % (options.config, chr1, chr2, options.name, chr1.replace(':', '_'), chr2.replace(':', '_'), linksToDraw, otherOptions))
					filesNames.append([options.name+"_"+chr1.replace(':', '_')+"_"+chr2.replace(':', '_'), margin])
					if chr1.replace(':', '_') in projList:
						if not chr2.replace(':', '_') in projList[chr1.replace(':', '_')]:
							projList[chr1.replace(':', '_')] += [chr2.replace(':', '_')]
					else:
						projList[chr1.replace(':', '_')] = [chr2.replace(':', '_')]
					if chr2.replace(':', '_') in projList:
						if not chr1.replace(':', '_') in projList[chr2.replace(':', '_')]:
							projList[chr2.replace(':', '_')] += [chr1.replace(':', '_')]
					else:
						projList[chr2.replace(':', '_')] = [chr1.replace(':', '_')]
		else:
			for chrom in chrList:
	...
	pool = mp.Pool(processes=threadsNb)
	results = pool.map(drawCircos, listJobs)

	for resultJob in results:
		if resultJob:
			raise ValueError("There is a bug in one job. Please read the error log.")

	pool2 = mp.Pool(processes=threadsNb)
	results2 = pool2.map(extendPolygon, filesNames)

	#Manage the repertories
	if options.erase:
		if os.path.isdir('./'+options.name):
			shutil.rmtree('./'+options.name)
	if not os.path.isdir('./'+options.name):
		shutil.copytree(os.path.dirname(os.path.realpath(__file__))+'/template/', './'+options.name+'/template/')
		os.mkdir('./'+options.name+'/img')

	jsonFile = './'+options.name+'/template/js/areas.json'
	createJS(jsonFile, results2)
	with open(options.name+'/template/js/plist.json', 'w') as outfile:
		outfile.write('plist = \n')
		json.dump(projList, outfile, sort_keys=True)

	for fileName in filesNames:
		shutil.copy(fileName[0]+'.png', './'+options.name+'/img/')
		os.remove(fileName[0]+'.html')
		os.remove(fileName[0]+'.png')
	createHtmlTemplate(options.name, options.config, chrList)
	if os.path.isfile(options.name+'.zip'):
		os.remove(options.name+'.zip')
	shutil.make_archive(options.name, 'zip', './'+options.name)
